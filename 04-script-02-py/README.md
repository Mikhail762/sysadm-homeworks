...
Переменная `c` определена не будет, так как попытка сложения числа a и строки b приведет к ошибке.
...
path = os.popen("echo -n $HOME").read() + r'/netology/sysadm-homeworks'
bash_command = [f"cd {path}", "git status"]
result_os = os.popen(' && '.join(bash_command)).read()
print(path)
for result in result_os.split('\n'):
  if result.find('modified') != -1:
    result = result.replace('\tmodified:   ', '')
    print(result)
```
...
if len(sys.argv) == 1:
  ...
  if not(os.path.exists(path)):
    sys.exit('Неправильный путь: '+path)
...
if os.path.exists(path+'.git'):
  ...
  print(path, ' Не является репозиторием')
```  
